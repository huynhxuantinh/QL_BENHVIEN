...
`Hospitals`:
- Bắt buộc: `ten, lat, lon`
- Nên có: `dia_chi, phuong, co_bhyt, cap_cuu_24h, loai_hinh, gio_mo, gio_dong`
- Ảnh bệnh viện (tùy chọn):
  - `hinh_anh_paths` (khuyến nghị): nhiều ảnh trong 1 ô, phân tách bằng `|` hoặc `;`
  - Có thể dùng các tên cột tương đương: `hinh_anh`, `duong_dan_anh`, `image`, `image_path`, `image_url`
  - Ví dụ giá trị:
    - `BENH_VIEN/Benh vien A/a1.jpg | BENH_VIEN/Benh vien A/a2.jpg`
...
Lưu ý import:
- Đường dẫn ảnh tương đối được resolve theo:
  1) thư mục chứa file `data.xlsx`,
  2) thư mục cha của project,
  3) thư mục hiện tại khi chạy lệnh.
- Nếu ảnh không tồn tại ở các đường dẫn trên, command sẽ báo `Images skipped`.
- Nếu bệnh viện chưa có ảnh đại diện, ảnh import đầu tiên sẽ được gán làm ảnh đại diện.
- Chỉ dùng một cột cấp cứu duy nhất: `cap_cuu_24h`.
